...
def ensure_connection(func):
    def wrapper(self, *args, **kwargs):
        ...
        if not _db.url:
            ...
            _db.url = DATABASE_URL
            _db.driver, _db._database_name = set_connection(DATABASE_URL)
...
def clear_neo4j_database(db, clear_constraints=False, clear_indexes=False):
    import neomodel_plus.neomodel.core as core
...
def set_connection(url):
    ...
    driver = GraphDatabase.driver(u.scheme + '://' + hostname, **options)
...
class Database(local, NodeClassRegistry):
    ...
    def cypher_query(self, query, params=None, handle_unique=True, retry_on_session_expire=False,
        ...
        """
        if not self.session:
            self.session = self.driver.session(database=self._database_name)
    ...
    def begin(self, access_mode=None):
        ...
        self._active_transaction = self.driver.session(default_access_mode=access_mode,
                                                       database=self._database_name).begin_transaction()
