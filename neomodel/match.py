...
class NodeSet(BaseSet):
    ...
    def _get(self, limit=None, lazy=False, **kwargs):
        if 'id' in kwargs:
            query = f"MATCH (n:{self.model.__label__}) WHERE ID(n)=$id RETURN n"
            results, _ = db.cypher_query(query, {"id": int(kwargs['id'])}, resolve_objects=True)
            if results:
                return [n[0] for n in results]
            return []
        self.filter(**kwargs)
    ...
    def order_by(self, *props):
        ...
        remove ordering.
        """
        should_remove = len(props) == 1 and props[0] is None or len(props) == 2 and props[0] == 'pk'
